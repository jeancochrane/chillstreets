...
public class ChillstreetsBundle implements ConfiguredBundle<GraphHopperBundleConfiguration> {
    ...
    private void runRegularGraphHopper(CmdArgs configuration, Environment environment) {
        final ChillstreetsManaged graphHopperManaged = new ChillstreetsManaged(configuration, environment.getObjectMapper());
        environment.lifecycle().manage(graphHopperManaged);
        environment.jersey().register(new AbstractBinder() {
            @Override
            protected void configure() {
                bind(configuration).to(CmdArgs.class);
                bind(graphHopperManaged).to(ChillstreetsManaged.class);
